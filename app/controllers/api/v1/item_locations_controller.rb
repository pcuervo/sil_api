class Api::V1::ItemLocationsController < ApplicationController
  respond_to :json
  ...
  def create
    ...
    if params[:inventory_item_id].nil? or params[:inventory_item_id].empty? 
      render json: { errors: 'No se ha encontrado el artículo.' }, status: 422
      return
    end

    if params[:warehouse_location_id].nil? or params[:warehouse_location_id].empty? 
      render json: { errors: 'No se ha encontrado la ubicación.' }, status: 422
      return
    end

    item_location = ItemLocation.new
    item_location.units = params[:units]
  ...
  def get_details
    ...
    if item_location.present?
      render json: item_location.get_details, status: 201, location: [:api, item_location]
      return
    end
